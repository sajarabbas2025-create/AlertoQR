...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (error || !data) return res.status(200).json({ success: false, error: "Profile nahi mili." });

    // 2. Formatting (SMSCountry ko '91' prefix ke saath 12 digits chahiye)
    ...
    // 3. SMSCountry Bridge Payload (Official Format)
    // Kuch API versions mein 'Number' parameter hi dono numbers ko comma se jod kar maangta hai
    const payload = {
        "Number": `${helper},${owner}`,
    ...
    return res.status(200).json({ success: response.ok, message: result });
